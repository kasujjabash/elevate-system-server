...
let DashboardService = class DashboardService {
  ...
  async getGenderBreakdown(hubId) {
  ...
  async getCompletionByStatus(hubId) {
  ...
  async getCourseCompletion(hubId, courseId) {
  ...
  async getGradeRollup(hubId, courseId) {
    const gradedSubs = await this.prisma.submission.findMany({
      where: {
        status: { in: ['Graded', 'Returned'] },
        score: { not: null },
  ...
  async getActiveStudentsList(hubId, courseId, limit = 100) {
  ...
  async getInactiveStudentsList(hubId, courseId, limit = 100) {
  ...
  async getRecentlyRegisteredStudents(hubId, limit = 10) {
    const students = await this.prisma.student.findMany({
      where: hubId ? { hubId } : undefined,
      orderBy: { createdAt: 'desc' },
      take: limit,
  ...
  async getPlacementBreakdown(hubId, courseId, limit = 10) {
  ...
  async getTotalWagesThisYear(hubId, courseId) {
    const now = new Date();
  ...
  async getAdminReport(hubId, courseId) {
    const [
      stats,
      hubStats,
      reportStats,
